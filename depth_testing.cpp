...
// settings
const unsigned int SCR_WIDTH = 1200;     // 1200 1920 800  512
const unsigned int SCR_HEIGHT = 700;    // 700 1080 600  400
const unsigned int ENV_SIZE = 512; // 正方形分辨率 用于dynamic skybox的framebuffer
glm::vec3 dynamicCubeCoordinate = glm::vec3(0.0, 0.1f, 3.5f);      // 
...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 envCubePorjection = glm::perspective(glm::radians(90.0f), 1.0f, near, far);
        glBindFramebuffer(GL_FRAMEBUFFER, envFramebuffer);
...
void processInput(GLFWwindow *window)
{
    ...
    if(glfwGetKey(window, GLFW_KEY_UP)==GLFW_PRESS) {
        dynamicCubeCoordinate.z -= 0.1f;
    }
    if(glfwGetKey(window, GLFW_KEY_DOWN)==GLFW_PRESS) {
        dynamicCubeCoordinate.z += 0.1f;
    }
    if(glfwGetKey(window, GLFW_KEY_LEFT)==GLFW_PRESS) {
        dynamicCubeCoordinate.x -= 0.1f;
    }
